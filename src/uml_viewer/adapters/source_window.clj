...
(defn source-lines->html
  ...
  ([source highlight-line]
   (source-lines->html source highlight-line :clojure))
  ([source highlight-line lang]
   (let [lines (str/split (or source "") #"\r?\n" -1)]
     (->> lines
          (map-indexed (fn [idx line]
                         (let [n (inc idx)
                               hl? (and highlight-line (= n highlight-line))
                               line-html ((if (= lang :clojure) colorize-clojure-html html-escape)
                                          (expand-tabs line))
...
(defn source->html
  ...
  ([title source highlight-line]
   (source->html title source highlight-line :clojure))
  ([title source highlight-line lang]
   (str "<html><head><style>"
        ...
        "<div class='src'><table>" (source-lines->html source highlight-line lang) "</table></div>"
        "</body></html>")))

(defn- build-frame!
  [title body line lang]
  (let [frame (JFrame. title)
        editor (JEditorPane. "text/html" (source->html title body line lang))
...
(defn open-member-window!
  ...
  ([source-impl ident]
   (when-let [{:keys [title body line lang]} (source/member-source (or (:lang ident) source-impl) ident)]
     (SwingUtilities/invokeLater
       (fn []
         (build-frame! title body line (or lang :clojure))))
     true))
